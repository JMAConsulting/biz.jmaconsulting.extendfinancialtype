  ...
  private function processContribution(&$params) {
    ...
    if (isset($params['field'])) {
      foreach ($params['field'] as $key => $value) {
        ...
        // JMA assign chapter and fund.
        $chapterCredit = CRM_Utils_Array::value(CHAPTER_CREDIT, $value, NULL);
        $fundCredit = CRM_Utils_Array::value(FUND_CREDIT, $value, NULL);
        $trxnChapterFund = [
          'chapter_code_trxn' => CRM_Utils_Array::value(CHAPTER_DEBIT, $value, NULL),
          'fund_code_trxn' => CRM_Utils_Array::value(FUND_DEBIT, $value, NULL),
        ];
        // JMA assign chapter and fund end.
        ...
        // JMA Do chapter and fund stuff processing.
        ...
        // JMA End chapter and fund processing.
  ...
  private function processMembership(&$params) {
    ...
    if (isset($params['field'])) {
      ...
      foreach ($params['field'] as $key => $value) {
        ...
        // JMA assign chapter and fund.
        $chapterCredit = CRM_Utils_Array::value(MEM_CHAPTER_CREDIT, $value, NULL);
        $fundCredit = CRM_Utils_Array::value(MEM_FUND_CREDIT, $value, NULL);
        $trxnChapterFund = [
          'chapter_code_trxn' => CRM_Utils_Array::value(MEM_CHAPTER_DEBIT, $value, NULL),
          'fund_code_trxn' => CRM_Utils_Array::value(MEM_FUND_DEBIT, $value, NULL),
        ];
        // JMA assign chapter and fund end.
        ...
        if (!empty($params['member_option']) && CRM_Utils_Array::value($key, $params['member_option']) == 2) {
          ...
          $contribution = CRM_Member_BAO_Membership::recordMembershipContribution($contrbutionParams);
          ...
          if (!empty($chapterFund)) {
            CRM_EFT_BAO_EFT::addChapterFund($chapterFund[0]['chapter_code'], $chapterFund[0]['fund_code'], $membership->id, "civicrm_membership");
          }
          else {
            CRM_EFT_BAO_EFT::addChapterFund($chapterCredit, $fundCredit, $membership->id, "civicrm_membership");
          }
          // JMA chapter fund processing for membership end.
        }
        else {
          $dateTypes = array(
            'join_date' => 'joinDate',
